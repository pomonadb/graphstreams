...
The QIElem imposes an order on the edge, represnted by the member boolean
...
\todo{implement temporal sorting condition in QISequence constructor. Perhaps
  experimentation to determine the optimal filtering order.}.
...
\todo[inline]{State InitialEdges Algorithm}
...
actually implemented... \todo{implement implicit semantics check... duh}
...
\begin{algorithm}
  ...
  \ForEach{ $(e,v) \in$ alist }{
   ...
   \If{\iso.\texttt{viso}$[T.v] \geq 0$} {
    ...
    \lIf{not $\impBool(T.\impVar)(\iso \cup e)$} {\textbf{continue}}
