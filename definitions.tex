...
$\T : E \to N^2$ is a function retrieve the active period for an edge $e \in E_G$, $\T(e) = (\ts_e, \tf_e)$.
...
\begin{defn}
Given a graph $G = (V_G, E_G, L_G)$ and a set of edges $S_e \subseteq E_G$, we say that $\pi_{S_e}(G) = (V_{S_e}, S_e, L_G)$, where $V_{S_e} = \bigcup_{e \in S_e} \{\pi_1(e)\} \cup \{\pi_2(e)\}$, is the edge-induced graph of $S_e$ on $G$. 
\end{defn}
