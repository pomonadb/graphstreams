...
We can also consider a slightly different model for this algorithm, considering
these map $dist$ to be a matrix (or equivalently a database table) and then
introducing a temporal adjacency matrix.

\begin{defn}
  For a temporal graph $G = (V, E, \T)$, where $V$ is the vertex set, $E$ is the
  edge set, and $\T : E \to \TT^2$ is the temporal activity function.  Let
  $A_\TT = \left(a_{uv}\right) \in M_n(\P{\TT^2})$ be the
  temporal adjacency matrix of $G$, where $a_{uv}$ is the set of all pairs of
  start and end times such that there is an edge bewteen $u$ and $v$.
  \todo[inline]{It's possible that $A_\TT \in M_n(\TT^2)$, but I'm not certain
    whether we want this to exist only at time $t \in \TT$, or if we want this
    to be a universal matrix}
...
\KwOut{The algorithm outputs $\tau$ such that
  ...
  \ForEach(//loop over nodes in time slices){
    ...
    \For{ $i = 0$ \KwTo $l$  }{
      \ForAll(the non-null entries in the row)
      {$(d_0, (I_u)) \in \tau_t[v_{src}]$ at vertex $u$}{
        \ForAll{$I, I_u \cap I \neq \emptyset, I \in A_\TT[v_{src}][u]$}{
          $\tau_t[t_0][v_{src}][u] = (d_0 + 1, I_u \cap I)$\;
        }
      }
    }
  }
