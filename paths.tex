...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{A temporal graph $G = (V, E, T)$ and $\tau$ which encapsulates
  a complete mapping between pairs of vertices along with an initial timestamp
  to the distance between them, with the default values set to $\emptyset$, and a 
  length $l$ which specifies the max distance to search.}
\KwOut{The algorithm outputs $\tau$ such that
  $dist[t][v_{src}][v_{dest}] = d$ where $d$ is the (shortest consecutive path)
  distance from node $v_{src}$ to node $v_{dest}$ such that time $t$ intersects the time
  interval of the first edge in the path.}
  \ForEach{
    time, start node pair $(t, v_{src})$ in $\TT \times V_t$
  }{
    Let $R$ := $\{v_{src}\}$\;
    Let $S$ := $\emptyset$\;
    \ForEach{$u \in R$}{
      \ForEach{$(d, I_{v_{src},u}) \in \tau_t[v_{src}][u]$}{
         Let $S_I$ := $A_\TT[u]$\;
         \ForEach{$I \in S_I$ at vertex $u$}{
           \ForEach{$i \in I$}{
             \If{$i \cap I_{v_{src},u}$}{
               $\tau_t[v_{src}][u']$ \texttt{+=} $(d,i)$ \;
               \If{$d+1 < l$}{
                  $R$ \texttt{+=} $u'$ \;
               }
             }
  ...
  \Return{dist}\;
 \caption{Consecutive Shortest Path Distance Algorithm}
\label{fig:matrix_consec_STP_alg}
\end{algorithm}

The efficiency of this is
$O(ltn^{\log_{\Delta G}n}\log^3m) = O(ltn\log^3m)$, so it seems
that this algorithm, in fact, fares similarly to the previous one,
Solely dependent on whether  

Unfortunately, this is still doing a bunch of double counting, and we
want to find a way, so that when we get to a node $v$ from time $t$
for which we have calculated a distance tree in $t - 1$, and $t$
intersects with the active interval of the subtree, we can skip that
subtree.  One way to do this is by running dijkstra on the
co-incidence graph.

\begin{defn}
  For a temporal graph $G = (V,E,T)$, its \textbf{coincidence graph},
  $C_G = (E,\mathcal{E})$, with $\mathcal{E} \subseteq E^2$ is a
  simple graph where the edges of $G$ become nodes of $C_G$, and there
  is an edge between two nodes exactly when they share an incident
  vertex in $C_G$. Symbolically, for two edges $e = uv, f = wz$ in the
  graph $G$, there is an edge $ef \in \mathcal{E}$ in the graph $C_G$
  if and only if $\{u,v\}\cap \{w,z\} \neq \emptyset$.
\end{defn}

The great thing about this coincidence matrix is that it is simply
another way of looking at existing data, and doesn't require addtional
computation to create. This gives rise to another version of this
algorithm. It actually requires two steps.  The first is to run
dijkstra on $C_G \cap I_G$, so that the edges are co-incident and
contemporary (we won't actually pre-compute this graph, but we'll do
it as we go, to allow for one(ish) pass). 
 

\begin{algorithm}[H]
  \SetAlgoLined 

  \KwIn{A temporal graph $G = (V, E, T)$ and $e\_dist$ which
    encapsulates a complete mapping from pairs of edges to the
    distance between them, with the default value set to $\infty$}

  \KwOut{The algorithm outputs $e\_dist$ such that
    $e\_dist[e_{src}][e_{dest}]$ is the distance between $e_{src}$ and
    $e_{dest}$ in $C_G$.}  
...
The above algorithm is simply a BFS so it has worst-case time
complexity described by $O(|E| + |\mathcal{E}|)$.

Once this step has completed, we must convert this intermediate
result, the shortest paths on $C_G$, to the one we are searching for,
the shortest paths on $G$. To do this, for every pair of vertices,
find the smallest distance in $C_G$ between the incident edges, and
add one, we see this in Algorithm~\ref{fig:edge_to_dist_alg}

\begin{algorithm}[H]
  \SetAlgoLined
  
  \KwIn{A graph $G = (V,E,T)$, where $T : E \to \TT$ and the
    precomputed map $e\_dist$ describing the distances between all
    vertices in $C_G = (E, \mathcal E)$.}
  
  \KwOut{The map $dist$ describing the distances between all pairs of
    vertices at every time slice. }


  \ForEach{$(t,u,v) \in \TT \times V_t \times V$}{
    $d_e := \min\{e\_dist[e][f] | e \in N_E(u), f \in
    N_E(v), t \in T(e)\}$ \;
    $dist[t][u][v] :=  d_e + 1$ \;
  }
  \Return{dist}\;
...
So the overall time complexity of algorithms \ref{fig:edge_consec_BFS}
and \ref{fig:edge_to_dist_alg} composed is
$O(tn^2\Delta^2 + m + |\mathcal{E}|) = O(tn^2\Delta^2 +
|\mathcal{E}|)$, and for a sparse graph $O(tn^2)$.
