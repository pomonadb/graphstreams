...
Note that there is a direct transformation between undirected temporal graphs
and directed temporal networks. To go from undirected to directed, for every
edge $(e_l,e_r) \in E^2$, add edges $e_l \in E$ and $e_r \in E$ to the directed
...
between pairs of vertices and the distance between them, with the default
...
worst case, every edge will enter $R$ at some point. So we have $O(nm\log m)$.
  ...
  for src in V:
    ...
    while R != {}:
      ...
      for e in edge_neighborhood(pred_e) - union S R:
        if (intersect(T(pred_e),T(e)) != null)
          dist[src][fst(e)] := dist[src][snd(e)] + 1
          insert(e,R)
      S += pred_e
    return dist
\end{lstlisting}
\caption{An algorithm to compute the lengths of all consecutive temporal shortest
paths}
...
\begin{lstlisting}[frame = single]
  ...
  for src in V:
    R := {(src,src)}
    S := {}
    interval[] = repeat null |V| times.
    dist[src][src] = 0
    while R != {}:
      pred_e := pop R
      for e in edge_neighborhood(pred_e) - union S R:
        if (intersect(interval[pred_e],T(e)) != null)
          dist[src][fst(e)] := dist[src][snd(e)] + 1
          interval[e] := intersect(interval[pred_e],T(e))
          insert(e,R)
        S += pred_e
    return dist
\end{lstlisting}
\caption{An algorithm to compute the lengths of all complete temporal shortest
paths}
...
first edge, and the maximum of the start times of all edges.
...
\begin{figure}[H]
  ...
  for src in V:
    R := {(src,src)}
    S := {}
    time[src][src] = (-Inf, Inf)
    while R != {}:
      pred_e := pop R
      for e in edge_neighborhood(pred_e):
        time' : = intersect(T(pred_e),T(e))
        if (interval != null):
          (ts,tf) := time'
          (rs,rf) := time[src][fst e]
          if rs = -Inf :
            time[src][snd e] := time'
          else:
            if ts > rf:
        ...
        insert(e,R)
      S += pred_e
    return matrix of the magnitudes of every entry in time
...
\begin{figure}[H]
  ...
  for src in V:
    ...
    while R != {}:
      pred_e := pop R
      for e in edge_neighborhood(pred_e):
        time' := intersect time[src][fst e] T(e)
        if time' != null
          if |time'| < |time[src][fst e]|
            time[src][snd e] := time'
          else
            time[src][snd e] := time[src][fst e]
        insert(e,R)
      S += pred_e
    return matrix of the magnitudes of every entry in time
