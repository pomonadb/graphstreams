...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{A temporal graph $G = (V, E, T)$ and a 3D array $dist$ which encapsulates
...
\KwOut{The algorithm outputs the 3D array $dist$ such that
  $dist[t_i][src][dest] = d$ where $d$ is the (shortest consecutive path)
  distance from node $src$ to node $dest$ such that time $t_i$ intersects the time
  interval of the first edge in the path.}
  \ForEach(//loop over nodes in time slices){
    start node $(t, v_{src})$ in $V \times \mathcal{T}$
  }{
    Let $e_{start} = (v_{src}, v_{src})$\;
    Let shortest interval be $T(e_{start}) = (t, t)$\;
    Let the reached edges min-heap $R = \{e_{start}\}$\;
    Let the searched edges $S = \varnothing$\;
    Let $dist[t][src][src] = 0$\;
    \While{$R \neq \varnothing$}{
      Let $e_{pred} = pop(R)$\;
      \ForEach{$e_{succ} = (u,v) \in N_{e_{pred}}(v) - \{e_{succ}\} - S \cup R$}{
        \If{$T(e_{pred}) \cap T(e_{succ}) \neq \varnothing$}{
          Let $dist[t][src][v] = min(dist[t][src][u] + 1, dist[t][src][v])$\;
...
\KwIn{A temporal graph $G = (V, E, T)$ and a 3D array $dist$ which encapsulates
  ...
  to the distance between them, with the default values set to $\infty$.}
\KwOut{The algorithm outputs the 3D array $dist$ such that
  $dist[t_i][src][dest] = d$ where $d$ is the (shortest complete path)
  distance from node $src$ to node $dest$ such that time $t_i$ intersects the time
  interval of the first edge in the path.}
  \ForEach(//loop over nodes in time slices){
    start node $(t, v_{src})$ in $V \times \mathcal{T}$
  }{
    Let the reached edges min-heap $R = \{(v_{src}, v_{src})\}$\;
    Let the searched edges $S = \varnothing$\;
    Let $dist[t][src][src] = 0$\;
    \For{$i$ in $range(|V|)$}{
      Let $interval[t][i] = null$\;
    }
    \While{$R \neq \varnothing$}{
      Let $e_{pred} = pop(R)$\;
      \ForEach{$e_{succ} = (u,v) \in N_{e_{pred}}(v) - \{e_{succ}\} - S \cup R$}{
        \If{$(\{t\}\cap(interval[t][e_{pred}] \cap T(e_{succ}))) \neq \varnothing$}{
          Let $dist[t][src][v] = min(dist[t][src][u] + 1, dist[t][src][v])$\;
          Let $interval[t][e_{succ}] = interval[t][e_{pred}] \cap T(e_{succ})$\;
          Insert $e_{succ}$ into $R$\;
        }
      }
      Insert $e_{succ}$ into $S$\;
    }
  }
  \Return{dist}\;
 \caption{Complete Shortest Path Distance Algorithm}
\label{fig:comp_STP_alg}
\end{algorithm}

\begin{proof}
  The inductive proof follows by the proof of BFS and the definition of complete
  shortest path.
\end{proof}

\section{Fastest Path Algorithms}

These algorithms rely on a slightly different (and more correct) definition of
fastest temporal path, where the time taken by a path is the absolute value of
the difference between the latest start time within the active interval of the
first edge, and the maximum of the start times of all edges. In the following
algorithm, the lower bound is fixed by the initial starting time, so all we care
about is the upper bound on the interval, because the time a $t,u,v$-path takes
is the upper bound of the interval taken by all of the paths less $t$.
...
\begin{figure}[H]
  ...
  \end{lstlisting}
  \caption{An algorithm to compute the times of all consecutive fastest paths}
  \label{fig:consec_FTP_alg}
\end{figure}

\begin{proof}
  The inductive proof follows by the proof of Dijkstras Algorithm replacing the
  definition of distance with the notion of shortest time.
\end{proof}
...
\begin{figure}[H]
  ...
  \end{lstlisting}
  \caption{An algorithm to compute the times of all complete fastest paths}
  \label{ref:comp_FTP_alg}
\end{figure}

\begin{proof}
  The proof follows from the proof of Dijkstras algorithm, with the appropriate
